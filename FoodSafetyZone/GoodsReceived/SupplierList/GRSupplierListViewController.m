//
//  GRSupplierListViewController.m
//  FoodSafetyZone
//
...
@interface GRSupplierListViewController ()
{
    int searchtag;
}
...
@implementation GRSupplierListViewController
...
-(void)singleTap:(UITapGestureRecognizer *)position
{
    ...
    if([filteredListContent count]==0)
    {
        ...
        status = [dbmanager.fmDatabase  executeUpdate:@"UPDATE temp SET tempStorageValue = ?, tempArray = ? WHERE id = 1",[NSMutableString stringWithFormat:@"%@",[allIDS objectAtIndex:[position.accessibilityLabel intValue]]],[NSMutableString stringWithFormat:@"%@",[listContent objectAtIndex:[position.accessibilityLabel intValue]]]];
    }
    else
    {
    ...
    {    // The iOS device = iPhone or iPod Touch
        ...
        {
            GRCategoryListViewController *categoryList = [[GRCategoryListViewController alloc]initWithNibName:@"GRCategoryListViewController" bundle:nil];
            [self.navigationController pushViewController:categoryList animated:YES];
            categoryList = nil;
        }
        if (iOSDeviceScreenSize.height > 480)
        {
            GRCategoryListViewController *categoryList = [[GRCategoryListViewController alloc]initWithNibName:@"GRCategoryListViewController5" bundle:nil];
            [self.navigationController pushViewController:categoryList animated:YES];
            categoryList = nil;
        }
    }
