...
-(void)loadDataForPage
{
    ...
    dbmanager.fmResults=[dbmanager.fmDatabase executeQuery:@"SELECT * FROM activityLogFoodItem"];
...
-(void)singleTap:(UITapGestureRecognizer *)position
{
    ...
    {    // The iOS device = iPhone or iPod Touch
        ...
        {
            ALAddRecordViewController *addRecords = [[ALAddRecordViewController alloc]initWithNibName:@"ALAddRecordViewController" bundle:nil];
            [self.navigationController pushViewController:addRecords animated:YES];
            addRecords = nil;
        }
        if (iOSDeviceScreenSize.height > 480)
        {
            ALAddRecordViewController *addRecords = [[ALAddRecordViewController alloc]initWithNibName:@"ALAddRecordViewController5" bundle:nil];
            [self.navigationController pushViewController:addRecords animated:YES];
            addRecords = nil;
        }
    }
...
-(void)addClicked
{
    ...
    {    // The iOS device = iPhone or iPod Touch
        ...
        if (iOSDeviceScreenSize.height == 480)
        {
            ALAddFoodItemViewController *addFoodItem = [[ALAddFoodItemViewController alloc]initWithNibName:@"ALAddFoodItemViewController" bundle:nil];
            [self.navigationController pushViewController:addFoodItem animated:YES];
            addFoodItem = nil;
        }
        if (iOSDeviceScreenSize.height > 480)
        {
            ALAddFoodItemViewController *addFoodItem = [[ALAddFoodItemViewController alloc]initWithNibName:@"ALAddFoodItemViewController5" bundle:nil];
            [self.navigationController pushViewController:addFoodItem animated:YES];
            addFoodItem = nil;
        }
    }
