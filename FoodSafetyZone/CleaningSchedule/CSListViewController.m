...
@synthesize mainScroll,search,filteredListContent,listContent, savedSearchTerm, savedScopeButtonIndex, searchWasActive,filteredImagesList,ImagesList,timerStatus,filteredTimerStatus,allIDS,filteredIDS;
...
-(void)loadDataForPage
{
    ...
    dbmanager.fmResults=[dbmanager.fmDatabase executeQuery:@"SELECT * FROM cleaningEquipments"];
...
-(void)singleTap:(UITapGestureRecognizer *)position
{
    ...
    {    // The iOS device = iPhone or iPod Touch
        ...
        {
            CSAddRecordViewController *addRecords = [[CSAddRecordViewController alloc]initWithNibName:@"CSAddRecordViewController" bundle:nil];
            [self.navigationController pushViewController:addRecords animated:YES];
            addRecords = nil;
        }
        if (iOSDeviceScreenSize.height > 480)
        {
            CSAddRecordViewController *addRecords = [[CSAddRecordViewController alloc]initWithNibName:@"CSAddRecordViewController5" bundle:nil];
            [self.navigationController pushViewController:addRecords animated:YES];
            addRecords = nil;
        }
    }
...
-(void)addClicked
{
    ...
    {    // The iOS device = iPhone or iPod Touch
        ...
        if (iOSDeviceScreenSize.height == 480)
        {
            CSAddEquipmentsViewController *addEquipments = [[CSAddEquipmentsViewController alloc]initWithNibName:@"CSAddEquipmentsViewController" bundle:nil];
            [self.navigationController pushViewController:addEquipments animated:YES];
            addEquipments = nil;
        }
        if (iOSDeviceScreenSize.height > 480)
        {
            CSAddEquipmentsViewController *addEquipments = [[CSAddEquipmentsViewController alloc]initWithNibName:@"CSAddEquipmentsViewController5" bundle:nil];
            [self.navigationController pushViewController:addEquipments animated:YES];
            addEquipments = nil;
        }
    }
