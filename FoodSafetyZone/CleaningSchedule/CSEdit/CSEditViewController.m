...
-(void)doneClicked
{
    ...
    else
    {
        ...
        status = [dbManager.fmDatabase  executeUpdate:@"UPDATE cleaningEquipments SET equipmentName = ?, equipmentImage = ?  WHERE id = ?",equipmentNameField.text,imgData,[NSNumber numberWithInt:selectedID]];
        imgData = nil;
        if (status == YES)
        {
